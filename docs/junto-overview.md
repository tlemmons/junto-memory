...
- **Chain-depth + recency window.** Once a chain exceeds the configured depth cap, the live push notification is suppressed; the human is notified. Bypass: if either the sender or recipient has had a fresh human interaction within five minutes, the cap is waived for that send — preserving low latency during active human-driven sessions.
